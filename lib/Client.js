...
class Client extends EventEmitter {
    ...
    typing(id, toggle = true) {
        return this.post("me/messages", { recipient: { id: id.split('_')[1] }, sender_action: (toggle) ? "typing_on" : "typing_off" })
    }
    
    // Sends the mark_as_seen action
    seen(id) {
        return this.post("me/messages", { recipient: { id: id.split('_')[1] }, sender_action: 'mark_seen' })
    }

    // Sends message to recipient
    send(sender, value, options = null, persona, style) {
        if (value) {
            let message = typeof value === "string" || typeof value === "number" ? { text: value } : value
